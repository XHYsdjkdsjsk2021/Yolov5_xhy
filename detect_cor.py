...
class Detector(object):
    ...
    # 检测器主体
    def detect(self,weight_path,image):
        # 先加载权重
        ...
        return confident[0].item(),max_index[0].item()
...
def detect(save_img=False):
            ...
            if det is not None and len(det):
                ...
                for i in range(len(sum_box)):
                    ...
                    if t[0]-10 < 0:
                        ...
                        c = t[0]-10
                    if t[2]+10 > im0.shape[1]:
                        d = t[2]
                    else:
                        d = t[2]+10
                    im_crop = im0[a:b,c:d]
                    im_crop = cv2.cvtColor(im_crop,cv2.COLOR_BGR2RGB)
                    # cv2.imwrite('/data/haoyuan/yolov5_test_result_2/1.jpg',im_crop)
                    detector=Detector('large',num_classes=143)
                    a,b= detector.detect('/data/haoyuan/yolov5-master/weights/best_large.pkl',im_crop)
                    print(a,b)
    ...
    true_dir = os.listdir('/data/haoyuan/adv_test/')
    


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', type=str, default='weights/best_for_m_0.908.pt', help='model.pt path')
